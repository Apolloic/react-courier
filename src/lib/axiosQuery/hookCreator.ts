  ...
  return (args?: CallBackArgsType<T>) => {
    ...
    const axiosQuery = new AxiosQuery({
      ...
      options: {
        ...
        hasDefaultDto:
          axiosQueryObject?.options?.applyDefaultDto,
      },
    });

    const endPoint = getFinalEndPoint<T>(
      axiosQueryObject.endPoint,
      args?.urlParams!
    );
